...
import Plugin = webpack.WebpackPluginInstance;
...
export class WebpackWarPlugin implements Plugin {
  ...
  apply(compiler: Compiler) {
    ...
    const archiveBaseName =
      ...
      JSON.parse(readFileSync(resolve(context, "package.json")).toString())
        .name;
    const archiveName =
      extname(archiveBaseName) == ""
        ? `${archiveBaseName}.war`
        : archiveBaseName;
    ...
    compiler.hooks.afterEmit.tap(
      ...
      (compilation: { assets: { [name: string]: any } }) => {
        ...
        outStream.on("close", () => {
          ...
          console.log(bold("\n\tWAR Archive"));
          console.log(
            bold(green(`\t\t${archiveName}\t${archiveSize}\t[written]`))
          );
        });
